...
vector<vector<int>> newBoard(MAX_N, vector<int>(MAX_N, 0)); // 초기화에 사용할 보드
microInfo m[51];
...
int bfs(int startX, int startY, int mNum){
    ...
    while(!q.empty()){
        ...
        for(int d=0; d<4; d++){
            ...
            if(nxt.x < 0 || nxt.x >= target.h || nxt.y < 0 || nxt.y >= target.w) continue;
            if(!target.shape[nxt.x][nxt.y]) continue;
...
bool isSplit(int mNum){
    ...
    if(m[mNum].area != area) return true;
    return false;
}
bool isFit(int targetX, int targetY, int idx){
    microInfo curr = m[idx]; // 복사본
    
    for(int i=0; i<curr.h; i++){
        for(int j=0; j<curr.w; j++){
            if(!curr.shape[i][j]) continue;
            
            int x = targetX + i;
            int y = targetY + j;
            
            if(x < 0 || x >= n || y < 0 || y >= n) return false; // 격자 밖
            if(board[x][y] != BLANK) return false; // 다른 미생물 있음
        }
    }
    ...
    for(int t=1; t<=q; t++){
        ...
        for(int i=0; i<m[t].h; i++){
            for(int j=0; j<m[t].w; j++){
                ...
                if(board[currX][currY] != BLANK){ // 미생물 이미 존재하는 칸
        ...
        for(int idx=1; idx<t; idx++){
            if(m[idx].area > 0 && !isSplit(idx)) continue; // 폐기 X
        ...
        for(int idx=1; idx<=t; idx++){
            if(m[idx].dead) continue;
            v.push_back({-m[idx].area, idx});
        }
        sort(v.begin(), v.end());
        
        board = newBoard; // 보드 초기화
        
        for(pair<int, int> curr : v){
            int idx = curr.second;
            
            bool flag = false; // 적합한 자리 찾았는지
        ...
        for(int aIdx=1; aIdx<=t; aIdx++){
            for(int bIdx=aIdx + 1; bIdx<=t; bIdx++){
                if(isNeighbor(aIdx, bIdx)) res += m[aIdx].area * m[bIdx].area;
            }
        }
        ans.push_back(res);

...
void update(int node, int l, int r, int ql, int qr, int val) { // 구간 [qr, ql]에 val 더하기
    if (l > qr || r < ql) return; // 포함되지 X
...
long long query(int node, int l, int r, int tg) {
    if (l == r) return tree[node];

    int mid = (l + r) >> 1;
    long long res = tree[node];
    if (tg <= mid) res += query(node << 1, l, mid, tg);
    else res += query((node << 1) + 1, mid + 1, r, tg);
